...
Рисунки должны быть оформлены с помощью пакета \textsf{TikZ}\footnote{\url{https://www.overleaf.com/learn/latex/TikZ\_package}}
\begin{figure}[h]
	\centering
	\begin{tikzpicture}
	
	\filldraw[fill=none, black] (0.0,1.8) rectangle (3.0,2.2) node[](e) {};
	
	\filldraw[fill=none, black] (0.0,0.0) rectangle (3.0,1.5) node[](H) {};
	
	\filldraw[fill=none, black] (4.5,0.0) rectangle (4.8,1.5) node[](s) {};
	
	\node[right] at (1.1, 0.7) {\Huge $H$};
	\node[right] at (3.5, 0.7) {\Huge $=$};
	\node[] at (1.5, 2.0) {\Large $e$};
	\node[] at (4.6, 0.75) {\Large $s$};
...
Ссылки на литературу, упомянутую в лекции. Если используется BibTeX, необходимо приложить .bbl  файл.
